...
team_arr = ['SK Gaming', 'Liquid', 'Fnatic', 'Virtus.pro', 'Astralis', 'FlipSid3 Tactics', 'Gambit e-Sports', 'Natus Vincere', 'FaZe Clan', 'GODSENT', 'mousesports', 'North', 'OpTic Gaming', 'EnVyUs', 'G2 Esports', 'HellRaisers']
...
def team_name_autocomplete(name):
    ...
    if possible_len == 0:
        ...
        returnDict['error'] = 'Teams {}, all start with "{}", type more lazy ass'.format(",".join(possible_teams), name)
    return returnDict
...
class predictCog:
    ...
    async def display(self,ctx):
        ...
        for userId,arr in entries.items():
            name = discord.utils.find(lambda m: m.id == userId, ctx.message.channel.server.members).name
    ...
    async def ladder(self,ctx):
        ...
        for userId in users:
            discord_user = discord.utils.find(lambda m: m.id == userId, ctx.message.channel.server.members)
