...
class pollCog:
    ...
    async def create_poll(self, ctx, pollName, *args: str):
        ...
        if pollName.lower() in self.config.all():
            if self.config.get(pollName.lower()).active:
                ...
                # 2.5 sec timeout
                message = await self.bot.wait_for_message(author=ctx.message.author, timeout=2.5)
                if message is None:
    ...
    async def poll_results(self, pollName):
        ...
        for k in poll.responses:
            ...
            # make count 1 per user or total number of user responses?
            numResponses += len(userResponses)
